...
所有通用代码在这里，所有评估特定的代码在 nanochat 目录中并从这里导入。
...
# 简单得多，因为我们不需要采样。因此，我们可以一次处理多个批次，
# 只需检查正确答案选择的 logits。
...
def run_categorical_eval(task_object, tokenizer, model, batch_size, max_problems=None):
    ...
    letter_to_id_cache = {} # many letters will repeat often, let's save the tokenizer some work
                            # 许多字母会经常重复，让我们为分词器节省一些工作
    num_passed, total = 0, 0
    for i in range(ddp_rank, num_batches, ddp_world_size):
        i0, i1 = i * batch_size, min((i + 1) * batch_size, num_problems)

        # Prepare the batch of problems. They might all be of different length, so we pad/collate them.
        # 准备问题批次。它们可能长度不同，所以我们要填充/整理它们。
        ...
        answer_time_positions = [len(ids) - 1 for ids in prompt_ids] # where the last token is (and the predicted answer)
                                                                     # 最后一个 token 的位置（以及预测的答案）
        padded_prompt_ids = [ids + [bos] * (max_length - len(ids)) for ids in prompt_ids]
        prompt_ids = torch.tensor(padded_prompt_ids, dtype=torch.long, device=device)

        # Get the logits for the whole batch of conversations in parallel (efficiency win here)
        # 并行获取整批对话的 logits（效率优势）
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--source', type=str, required=True, help="模型来源: sft|rl")
    ...
    parser.add_argument('-b', '--batch-size', type=int, default=8, help='分类评估的批量大小')
